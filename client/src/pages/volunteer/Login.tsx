...
export function VolunteerLogin() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.role === 'ngo_volunteer' && data.volunteer) {
        setVolunteer(data.volunteer);
        navigate('/volunteer');
      } else if (data.role === 'ngo_admin') {
        toast.error('This is the Volunteer portal. Please use the NGO portal instead.');
      } else {
        toast.error('No volunteer profile found for this account.');
      }
    } catch (err: any) {
    ...
    <div className="min-h-screen bg-gray-950 flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        ...
        >
          ...
          <Link
            to="/"
            className="text-xs text-gray-600 hover:text-gray-400 mb-5 flex items-center gap-1 transition-colors"
          >
            <ArrowLeft size={12} /> Back to portal selection
          </Link>

          <h1 className="text-2xl font-bold text-white mb-1">
            {isSetupMode ? 'Set up Account' : 'Volunteer Login'}
          </h1>
          ...
          {!isSetupMode && (
            <div className="mt-6 text-center text-xs text-gray-500">
              Go to{' '}
              <Link to="/ngo/login" className="text-teal-400 hover:underline">
                NGO Admin login
              </Link>
            </div>
          )}
