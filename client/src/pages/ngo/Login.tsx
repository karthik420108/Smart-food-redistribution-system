...
export function NgoLogin() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Parallel fetch NGO and Volunteer profiles from local connection
      const [ngoRes, volRes] = await Promise.all([
        supabase.from('ngo_organizations').select('*').eq('user_id', authData.user.id).maybeSingle(),
        supabase.from('ngo_volunteers').select('*, ngo_organizations(org_name)').eq('user_id', authData.user.id).maybeSingle()
      ]);
